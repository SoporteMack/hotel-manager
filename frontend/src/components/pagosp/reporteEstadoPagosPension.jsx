  ...
  const cargarReporte = async () => {
    try {
      ...
      const datosProcesados = rawData.map((item) => ({
        ...item,
        mes_actual: item.mes_actual === null ? '' : item.mes_actual, // Mes Actual
        mes_anterior: item.mes_anterior === null ? '' : item.mes_anterior, // Mes Anterior
        mes_2: item.mes_2 === null ? '' : item.mes_2, // Hace 2 Meses
      }));
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  <th className="px-4 py-3 text-left text-sm font-semibold text-gray-700">Número de Pension</th>
