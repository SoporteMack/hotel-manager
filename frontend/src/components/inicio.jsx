...
function Dashboard() {
    ...
    const [ultimosPagos, setUltimosPagos] = useState([]); // inicializamos como array
                ...
                {ultimosPagos.length > 0 ? (
                    ultimosPagos.map((ingreso, idx) => (
                        ...
                        >
                            ...
                            <div className={`mt-2 sm:mt-0 sm:ml-4 font-semibold text-lg sm:text-xl ${getMontoColor(ingreso.monto)}`}>
                                ${ingreso.monto}
                            </div>
                ...
                {selectedPago && (
                    ...
                    >
                        ...
                        >
                            ...
                            <p><strong>Folio:</strong> {selectedPago.folio}</p>
                            <p><strong>Departamento:</strong> {selectedPago.contrato.departamento.descripcion}</p>
                            <p><strong>Fecha:</strong> {selectedPago.fechaPago}</p>
                            <p><strong>Cliente:</strong> {selectedPago.contrato?.persona?.nombrePersona || ""} {selectedPago.contrato?.persona?.apellidoPaterno || ""} {selectedPago.contrato?.persona?.apellidoMaterno || ""}</p>
